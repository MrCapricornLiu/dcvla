...
The language model processes $L=L_t+L_v$ tokens during the prefill stage. With reuse ratio $\rho_l$, the effective recomputed prefill query length at reuse-enabled layers becomes $L' = L_t + (1-\rho_l)L_v$.
Cached K/V states for reusable visual tokens remain available as attention context, so the attention score computation changes from $L\times L$ to $L' \times L$ rather than to a physically shortened $L' \times L'$ sequence.
Using the decoder-block cost
\begin{equation}
    C_{\text{LLM}}(L) \approx 4 L D_l^2 + 2 L^2 D_l + 3 L D_l M_l,
    \label{eq:llm_flops}
\end{equation}
...
\begin{split}
    ...
    + 2\big(L^2 - L' L\big)D_l \\
    &\quad + 3(\rho_l L_v)D_l M_l.
\end{split}
\end{equation}
The attention term can be equivalently written as $2L(L-L')D_l$.
