#!/usr/bin/env bash
# OpenVLA-OFT ablation: VLA/Vit cache on/off combinations across 5 parameter sets.

set -e

ts=$(date +"%Y%m%d_%H%M%S")
BASE_PATH="/home/lch/Documents/dcvla/results_oft/ablation_switches"
...
run_group() {
  ...
  for combo in "vla1_vit0" "vla0_vit1" "vla1_vit1"; do
    ...
    run_suite libero_spatial checkpoints/openvla-7b-oft-finetuned-libero-spatial "$logdir" "$use_vla" "$use_vit" "$metric" "$sim" "$gray" "$attn" "$static_top_k" "$kf"
    run_suite libero_object  checkpoints/openvla-7b-oft-finetuned-libero-object  "$logdir" "$use_vla" "$use_vit" "$metric" "$sim" "$gray" "$attn" "$static_top_k" "$kf"
    run_suite libero_goal    checkpoints/openvla-7b-oft-finetuned-libero-goal    "$logdir" "$use_vla" "$use_vit" "$metric" "$sim" "$gray" "$attn" "$static_top_k" "$kf"
    run_suite libero_10      checkpoints/openvla-7b-oft-finetuned-libero-10      "$logdir" "$use_vla" "$use_vit" "$metric" "$sim" "$gray" "$attn" "$static_top_k" "$kf"
  done
}

# 5 parameter sets (OFT)
run_group "p01_gray_004_k160_s120_kf5" gray_diff - 0.004 160 120 5
run_group "p02_gray_003_k160_s120_kf8" gray_diff - 0.003 160 120 8
run_group "p03_cos_0994_k100_s160_kf3" cosine 0.994 - 100 160 3
run_group "p04_cos_0996_k100_s160_kf5" cosine 0.996 - 100 160 5
run_group "p05_cos_0994_k120_s120_kf5" cosine 0.994 - 120 120 5
