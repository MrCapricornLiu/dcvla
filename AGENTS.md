...
- Spatial rollout checks on 2026-05-22:
  ...
  - Superseding reuse-only check on 2026-05-23, Spatial task 0, `1` warmup + `10` measured rollouts, padded LLM bucket graph enabled, both VE/LLM prune disabled: overhead-controlled baseline `9/10` success, avg wall `215.784 ms`, overhead `23.154`, ViT `21.423`, LLM `170.260`, action head `0.947`; reuse-only `9/10` success, avg wall `211.526 ms`, overhead `22.767`, ViT `22.534`, LLM `165.272`, action head `0.952`. Logs: `experiments/logs/EVAL-libero_spatial-openvla-2026_05_23-09_02_19--baseline_reuseonly_cmp10.txt` and `experiments/logs/EVAL-libero_spatial-openvla-2026_05_23-09_02_19--reuseonly_padded_bucket_cmp10.txt`. This is a more stable estimate than the earlier 5-rollout result: reuse-only improves total latency by about `2.0%` and LLM latency by about `2.9%`, but VE is about `1.1 ms` slower in this run.
