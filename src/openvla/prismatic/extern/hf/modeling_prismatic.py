...
class PrismaticVisionBackbone(nn.Module):
    ...
    def _apply_cached_patches(self, patches: torch.Tensor, *, is_fused: bool) -> torch.Tensor:
        ...
        if self._reuse_mask is None:
            return patches

        cached = self._cached_fused if is_fused else self._cached_primary
        if cached is None:
            return patches

        mask = self._reuse_mask
        if mask.numel() != patches.shape[1]:
            # Mask length mismatch; skip reuse to avoid shape errors.
            return patches

        device_mask = mask.to(device=patches.device)
        if not device_mask.any():
            return patches

        cached = cached.to(device=patches.device, dtype=patches.dtype)
        patches = patches.clone()
        patches[:, device_mask] = cached[:, device_mask]
        return patches
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs: str,
        ...
        if past_key_values is not None:
            if past_key_values.cache_position is not None and not torch.all(input_ids[:, -1] == 29871):
